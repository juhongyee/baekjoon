...
def make_lps(pat,lps):
    ...
    for i in range(1,len(pat)):
        #i개 일치 상황, ABACABAAC 에서 1개 상황이면 A하나 겹침, 2개 상황이면 AB겹침 그 때 8개 때의 lps를 만듦.
        while(pat[j]!=pat[i] and j>0): #같을 때까지 iter하고 j 옮김. 어떤 경계 다음의 index와 같을 떄까지
            j = lps[j] #lps는 개수이므로 그 경계 다음 index로 감. 왜 경계로 갈까?
            #ABACABABC에서 7개 겹치는 경우를 생각해보자. B를 확인했을 때 안맞으니 ABA를 확인한다는 것인데 그 ABA가 B 앞에도 존재한다.(LPS[j]는 j개 겹치는 상황 j+1번째 겹칠 때의 경계 index)
